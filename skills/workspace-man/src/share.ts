...
const shouldExclude = (relPath: string) => {
  ...
  if (parts.includes('dist') || parts.includes('build') || parts.includes('out')) return true;
  if (parts.includes('.turbo')) return true;
  if (parts.includes('.next') || parts.includes('.svelte-kit') || parts.includes('.nuxt')) return true;
...
const copyDir = (src: string, dest: string, base: string = src) => {
...
const generateTempSshKeyPair = async (tempDir: string) => {
...
type ShareInitResponse = {
  sshUrl: string;
  webUrl: string;
};

const callShareInit = async (name: string, publicKey: string): Promise<ShareInitResponse> => {
...
export const shareWorkspace = async (workspaceDir: string) => {
